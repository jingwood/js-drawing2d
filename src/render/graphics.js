...
export class Graphics2D {
  ...
  restoreTransform() {
    if (this.transformStack.length > 0) {
      this.currentTransform = this.transformStack[this.transformStack.length - 1]
    } else {
      this.currentTransform = new Matrix3().loadIdentity()
    }
  ...
  drawRoundRect(rect, cornerSize, strokeWidth = 1, strokeColor = 'black', fillColor = 'white', strokeStyle) {
    ...
    if (strokeWidth > 0 && strokeColor) {
      ...
      switch (strokeStyle) {
        case 'dash':
          ctx.setLineDash([strokeWidth, ctx.lineWidth])
          break
      }
  ...
  drawPolygon(points, strokeWidth, strokeColor, fillColor) {
    ...
    var p0 = points[0]
    if (Array.isArray(p0)) ctx.moveTo(p0[0], p0[1])
    else ctx.moveTo(p0.x, p0.y)

    for (var i = 1; i < points.length; i++) {
      var p = points[i]
      if (Array.isArray(p)) ctx.lineTo(p[0], p[1])
      else ctx.lineTo(p.x, p.y)
    }

    if (Array.isArray(p0)) ctx.lineTo(p0[0], p0[1])
